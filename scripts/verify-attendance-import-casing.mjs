#!/usr/bin/env node
/**
 * Fails CI if attendance component imports use folder casing that does not match git.
...
const COMPONENTS = 'src/features/attendance/components';
...
const patterns = [
  /(?:from|import)\s+['"]([^'"]*features\/attendance\/components\/([^/'"]+))/g,
...
if (errors.length > 0) {
  console.error('Attendance import casing check failed:\n');
  for (const e of errors) console.error(`  - ${e}`);
  process.exit(1);
}

console.log('Attendance component import casing OK (matches git PascalCase folders).');
