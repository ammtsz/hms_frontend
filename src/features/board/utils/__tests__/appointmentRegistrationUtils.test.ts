...
} from "../appointmentRegistrationUtils";
...
describe("appointmentRegistrationUtils", () => {
  ...
  describe("validateBasicInputs", () => {
    ...
    it("returns error when selected types is empty", () => {
      ...
      expect(result).toContain("select at least one appointment type");
    });
  ...
  describe("buildSchedulingFailureMessage", () => {
    ...
    it("returns generic message when no firstError", () => {
      ...
      expect(msg).toContain("Error creating 3 appointment(s)");
    });
  });
});
