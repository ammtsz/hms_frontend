...
export function validateBasicInputs(
  ...
  if (!name || selectedTypes.length === 0) {
    return "Please enter the patient name and select at least one appointment type.";
  }
  return null;
}
...
export async function validateDayNotFinalized(
  ...
  if (result.success && result.value?.isFinalized) {
    return "Day already finalized. No more appointments can be scheduled for this day.";
  }
  return null;
}
...
/** Fetch appointments for target date, or use context appointments if same date. */
export async function getAppointmentsForTargetDate(
  targetDate: string,
  appointmentsByDate: AppointmentByDate | null
): Promise<AppointmentByDate | null> {
  const currentContextDate = getTodayClinic();
  if (targetDate === currentContextDate) return appointmentsByDate;

  const response = await getAppointmentsByDate(targetDate);
  if (!response.success || !response.value?.length) return appointmentsByDate;

  return transformAppointmentWithPatientByDate(response.value, targetDate);
}
...
export function validateDuplicateSchedule(
  patientName: string,
  selectedTypes: string[],
  appointmentsForTargetDate: AppointmentByDate | null,
  targetDate: string,
  ...
  return `The patient "${patientName}" already has a scheduled consultation ${dateText}. Check the appointment list.`;
}
...
export function buildSuccessMessage(
  ...
  return `Schedule created successfully! ${selectedTypesCount} appointment(s) scheduled ${dateMessage} at ${SCHEDULED_TIME}.`;
}
...
export function buildNewPatientSchedulingFailureMessage(
  ...
  return `PATIENT CREATED successfully, but it was NOT POSSIBLE to schedule the appointment because ${reason}. Uncheck the 'New patient' option, select the patient from the list, and try scheduling again.`;
}
...
export function buildSchedulingFailureMessage(
  ...
  return `Error creating ${failedCount} appointment(s). Some may have been created successfully.`;
}
