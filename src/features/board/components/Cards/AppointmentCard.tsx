...
const AppointmentCard: React.FC<AppointmentCardProps> = React.memo(
  ({
    ...
    // Check if appointment is missed or cancelled
    ...
    const handleDelete = async (
      ...
      openCancellation(appointmentIds, patientName, selectedDate);
    };

    // Get all appointment IDs for this patient (grouped cards may have multiple)
    const appointmentIds =
      isExpandableTreatment && isExpanded
        ? groupedPatient?.appointmentIds ||
    ...
    const { treatmentsWithSessionRows, isLoading: loadingSessions } =
      useTreatmentsWithSessionRows(
        appointmentIds.length > 0 ? appointmentIds : null,
      );

    // For the delete button, get appointment IDs (for scheduled items)
    const deleteAppointmentIds =
      groupedPatient?.appointmentIds ||
      (patient.appointmentId ? [patient.appointmentId] : []);
    ...
    // Appointment board shows numeric priority order (1–5), not admin-configured labels.
    const priorityDisplay = patient.priority;

    // Handle card click - expansion for physiotherapy/tens, completion for assessment
    const handleCardClick = () => {
      if (isClickable && patient.appointmentId && patient.patientId) {
        onCompletedClick(patient.appointmentId, patient.patientId, patient.name);
        ...
        {status === "scheduled" &&
          ...
          !isMissedOrCancelled && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
                handleDelete(deleteAppointmentIds, patient.name);
              }}
