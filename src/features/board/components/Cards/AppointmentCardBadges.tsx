...
interface AppointmentCardBadgesProps {
  isMissed: boolean;
  isCancelled: boolean;
...
const AppointmentCardBadges: React.FC<AppointmentCardBadgesProps> = ({
  isMissed,
  isCancelled,
  isNextToBeAttended,
  groupedPatient,
    ...
    <>
      ...
      {isNextToBeAttended && !isMissedOrCancelled && (
        <span className="absolute top-1 right-1 text-red-700 text-xs px-1 py-0.5 rounded z-10">
          {APPOINTMENT_CARD_OVERLAY_LABELS.next}
        </span>
      )}

      {/* Treatment type appointments count (appointments on this card only) */}
      {groupedPatient &&
...
AppointmentCardBadges.displayName = "AppointmentCardBadges";

export default AppointmentCardBadges;
