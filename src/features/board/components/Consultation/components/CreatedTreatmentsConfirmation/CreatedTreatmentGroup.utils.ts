 ...
 * Group key: duration, plannedSessions, startDate.
 */
function getGroupKey(session: CreatedTreatment): string {
  const duration = session.durationMinutes ?? "none";
  return `${duration}-${session.plannedSessions}-${session.startDate}`;
}
...
export interface GroupedCreatedTreatment {
  /** First row in group; shared props (duration, plannedSessions, startDate) apply to the whole group */
 ...
 * Each group shares: duration, planned sessions, and start date.
 */
