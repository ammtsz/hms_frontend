...
jest.mock("@/features/board/components/Consultation/hooks/useFormHandler");
...
describe("usePostConsultationForm", () => {
  // Set timeout for all tests
  jest.setTimeout(5000);

  // Access mocked functions
  const mockUseFormHandler = jest.requireMock(
    "@/features/board/components/Consultation/hooks/useFormHandler",
  ).useFormHandler;
  ...
  const mockUsePostConsultationModal = jest.requireMock(
    "@/stores/modalStore",
  ...
  const mockUseNewlyScheduledAppointments = jest.requireMock(
    "@/api/query/hooks/usePatientQueries",
  ).useNewlyScheduledAppointments;
  const mockUseConsultationByAppointment = jest.requireMock(
  ...
  const mockUseScheduleReturnAppointment = jest.requireMock(
    "@/api/query/hooks/useConsultationQueries",
  ...
  beforeEach(() => {
    ...
    mockUsePostConsultationModal.mockReturnValue({
      isOpen: true,
      appointmentId: 1,
      patientId: 1,
    ...
    // Mock useNewlyScheduledAppointments hook
    mockUseNewlyScheduledAppointments.mockReturnValue({
      data: [],
      isLoading: false,
      error: null,
    });

    // Mock useTreatmentRecordByAppointment hook
    mockUseConsultationByAppointment.mockReturnValue({
    ...
    mockUseScheduleReturnAppointment.mockReturnValue({
    ...
    mockUseFormHandler.mockReturnValue({
      formData: {
        ...
        returnWeeks: 1,
        homeExercises: "Cat-camel and pelvic tilt",
    ...
    // usePostConsultationForm uses bulkCreateTreatments (not createTreatment)
  ...
  describe("Hook Initialization", () => {
    it("should initialize successfully", () => {
      const { result } = renderHook(() => usePostConsultationForm(), {
        wrapper: TestWrapper,
      });
  ...
  describe("Patient Data Loading", () => {
    it("should fetch patient data on mount", async () => {
      renderHook(() => usePostConsultationForm(), {
        wrapper: TestWrapper,
      });
    ...
    it("should pre-fill mainConcern from existing treatment record", async () => {
      ...
      // Note: The hook uses useLatestTreatmentRecordByPatient, not useTreatmentRecordByAppointment
      mockUseLatestConsultationByPatient.mockReturnValue({
        data: {
          id: 1,
          patientId: 1,
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle missing appointmentId", () => {
      mockUsePostConsultationModal.mockReturnValue({
        isOpen: true,
        // No appointmentId provided
        patientId: 1,
        currentTreatmentStatus: "T",
        isLoading: false,
        onComplete: mockOnComplete,
  ...
  describe("Form Validation", () => {
    ...
    it("should validate physiotherapy treatments", () => {
      ...
      const invalidDurationResult = validateFn({
        mainConcern: "Test complaint",
        patientStatus: "T",
        startDate: "2024-01-15",
        returnWeeks: 1,
      ...
      expect(invalidDurationResult).toBe(
        "Physiotherapy duration must be 30, 45, or 60 minutes",
      );
    });
    ...
    it("should validate tens treatments", () => {
      ...
      const invalidQuantityResult = validateFn({
        ...
        recommendations: {
          ...
          tens: {
            treatments: [
              { locations: ["Head"], quantity: 25, duration: 30, startDate: "2024-01-15" },
            ],
            startDate: "",
          },
          returnWhenTreatmentComplete: true,
    ...
    it("should validate general recommendations when noGeneralRecommendations is false", () => {
      ...
      expect(result).toBe(
        "Add at least one general recommendation (home exercises, pain management, or medications) or mark none apply",
      );
    });
  ...
  describe("Treatment Session Creation", () => {
    beforeEach(() => {
      // Reset mocks and ensure appointmentId and patientId are available
      mockUsePostConsultationModal.mockReturnValue({
        isOpen: true,
        appointmentId: 1,
        patientId: 1,
        ...
        recommendations: {
          ...
          physiotherapy: {
            treatments: [
              {
                locations: ["Head"],
                duration: 20, // Invalid duration (must be 30, 45, or 60)
                quantity: 5,
                startDate: "2024-01-15",
              },
            ],
    ...
    it("should handle missing appointmentId or patientId", async () => {
      mockUsePostConsultationModal.mockReturnValue({
        isOpen: true,
        // Missing appointmentId and patientId
        currentTreatmentStatus: "T",
        isLoading: false,
        onComplete: mockOnComplete,
      });
      ...
      await act(async () => {
        try {
          ...
          expect((error as Error).message).toBe(
            "Appointment ID is required for treatment submission",
          );
        }
      });
    });
  ...
  describe("Error Parsing", () => {
    ...
    it("should parse validation error messages", async () => {
      ...
      mockBulkCreateTreatments.mockRejectedValue(
        new Error("duration_minutes must not be greater than 60"),
      );
